...
let pauseMenu; // Объект меню паузы
...
function draw() {
    if (!isPaused) {
        ...
        gameObj.Enemy.forEach(enemy => {
            enemy.draw();
            enemy.move(); // Добавлено движение врагов
            enemy.death();
        });
        ...
        pauseMenu.draw(); // Отображение меню
    }
}
...
function drawCounters() {
    ...
    textSize(20);
    textAlign(RIGHT, TOP);
    text(`Монеты: ${coinCount}`, width - 10, 10);
    text(`Враги: ${enemyKillCount}`, width - 10, 35);
}
...
function togglePause() {
    isPaused = !isPaused;
    if (isPaused) {
        noLoop(); // Остановка draw()
        backgroundMusic.stop();
        pauseMenu.show();
        color_slider.hide(); // Скрытие главного слайдера (слева который)
    } else {
        loop(); // Возобновленип draw()
        pauseMenu.hide();
        backgroundMusic.loop();
        color_slider.show(); // Показ главного слайдера при игре
    }
}
...
class PauseMenu {
    constructor() {
        ...
        // Создание слайдера для меню
        this.slider = createSlider(0, 255, color_slider.value());
        this.slider.style('width', '160px');
        this.slider.position(this.x + 20, this.y + 50);
        this.slider.hide();

        // Кнопка Resume в меню
        this.resumeButton = createButton('Resume');
        this.resumeButton.position(this.x + 20, this.y + 100);
        this.resumeButton.mousePressed(() => {
            togglePause();
        });
        this.resumeButton.hide();

        // Кнопка Restart в меню
        this.restartButton = createButton('Restart');
        this.restartButton.position(this.x + 20, this.y + 140);
    ...
    draw() {
        // Затемнение фон при меню
        background(0, 150);

        // Меню в виде прямоугольника
        fill(200);
        ...
        text('Пауза', width / 2, this.y + 10);

        // Обновление громкости из слайдера меню
        let volume = this.slider.value() / 255;
        backgroundMusic.setVolume(volume);
        attackAudio.volume = volume;
        deathAudio.volume = volume;
...
function generateCollectableItems() {
    ...
    for (let i = 0; i < itemAmount; i++) {
        ...
        if (validPosition) {
            ...
            console.warn(`Нет позиции для предмета ${i} после ${maxAttempts} траев`);
        }
    }
}
...
function generateEnemies() {
    gameObj.Platforms.forEach(platform => {
        ...
        gameObj.Enemy.push(new Enemy(enemyX, enemyY, 20, 10, platform.x, platform.x + platform.w));
    });
}
...
function Item(cordX, cordY, radius) {
    ...
    this.collection = function() {
        if (this.active && dist(gameObj.Character.x, gameObj.Character.y, this.x, this.y) < this.r + gameObj.Character.r / 2) {
            this.active = false;
            this.x = -100;
            this.y = -100;
            coinCount++; // Увеличиваем счетчик монет
        }
    };
...
function Enemy(cordX, cordY, width, height, platformStart, platformEnd) {
    this.x = cordX;
    this.y = cordY;
    this.w = width;
    this.h = height;
    ...
    this.move = function() {
        if (!this.enemyDefeated) {
            ...
            if (this.x < this.platformStart + this.w / 2) {
                this.direction = 1;
            } else if (this.x > this.platformEnd - this.w / 2) {
                this.direction = -1;
            }
        }
    };
    ...
    this.death = function() {
        if (!this.enemyDefeated && (gameObj.Character.x + 65) > this.x && gameObj.Character.enemyAttackState === true && (gameObj.Character.y - 65) < this.y) {
            this.enemyDefeated = true;
            enemyKillCount++; // Увеличение счетчика
        }
    };
