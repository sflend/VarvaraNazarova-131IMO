...
let worldWidth = 2048; // Увеличениеп ширины мира

let gameObj = {
    Untouch: [
        new ground(0, 350, 2048, 200), //  Увеличиваем ширину земли
    ...
    Clouds: [
        ...
        new cloud(1200, 80, 80, 1, 5), 
        new cloud(1700, 100, 60, 1, 5) 
    ],

    CollectableItem: [
        new item(500, 320, 40),
        new item(250, 200, 40),
        new item(1300, 320, 40) 
    ],
    ...
    Character: new character(50, 350, 60, 4, 50, 325, 60, 5, true)
}
...
function draw() {
    // Обновление положения камеры на основе положения игрока
    updateCamera();
    ...
    // Сохранение состояния матрицы преобразований
    push();
    ...
    // Земля
    fill(34, 139, 34);
    rect(0, 350, worldWidth, height - 350); // Земля на всю ширину мира

    // Рисует игровые обьекты
    ...
    // Восстанавление матрицы преобразований
    pop();
}
    ...
    this.draw = function()
    {
        ...
        this.move = function()
        {
            if ((this.x - this.r) < worldWidth)
                ...
                this.x = (4-this.r); // Облако к левому углу
        }
...
function platform(cordX, cordY, width, lenght)
{
    this.x = cordX;
    this.y = cordY;
    this.w = width;
    ...
    this.draw = function()
    {
        ...
        stroke('green')
        fill("green");
        rect(this.x, this.y, this.w, this.lenght);
    }
...
function character(cordX, cordY, radius, moveSpeed, constX, constY, constRadius, jumpSpeed) {
    ...
    this.isCollidingWPlatform = function(platform) {
        return (
            this.x + this.r / 2 > platform.x && // Персонаж справа от левой стороны платформы
            this.x - this.r / 2 < platform.x + platform.w && // Персонаж слева от правой стороны платформы
            this.y + this.r >= platform.y && // Персонаж касается
            this.y < platform.y + platform.lenght //Персонаж выше нижней границы платформы
        );
    ...
    this.move = function() {
        let isOverCanyon = false; // Флаг стоит ли персонаж над каньоном

        // Проверка находится ли персонаж над каньоном
        ...
        if (isOverCanyon && !isOnPlatform) {
            ...
            if (this.y >= this.constY) { // Проверка на землю
                isGrounded = true;
                this.jumping = false;
            }
    ...
    this.grounded = function() {
        ...
        // Проверка на землю (constY)
        if (this.y > this.constY) {
            this.y = this.constY;
            this.jumpSpeed = 0;
            this.jumping = false;
        } else if (this.y < this.constY) { // B воздухе
            this.jumping = true;
        }
    };
